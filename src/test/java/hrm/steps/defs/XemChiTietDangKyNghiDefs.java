...
public class XemChiTietDangKyNghiDefs {
	...
	public void Nhap_XEM_CHI_TIET_DANG_KY_NGHI_nhu(String dataKey, List<List<String>> dataTable) throws Exception {
	...
	@And("^Kiem tra XEM CHI TIẾT ĐĂNG KÝ NGHỈ nhu \"([^\"]*)\"$")
	public void Kiem_tra_XEM_CHI_TIET_DANG_KY_NGHI_nhu(String dataKey, List<List<String>> dataTable) throws Exception {
			...
			if (!sNhanSu.isEmpty()) {
				if (sNhanSu.startsWith("@MARK")) {
					if (sNhanSu.contains("@MARK HASFOCUS@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "HASFOCUS/NOTFOCUS: Nhân sự", true,
								XCTDKN.focus_nhan_su());
					} else if (sNhanSu.contains("@MARK NOTFOCUS@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "HASFOCUS/NOTFOCUS: Nhân sự", false,
								XCTDKN.focus_nhan_su());
					} else if (sNhanSu.contains("@MARK VISIBLE@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "VISIBLE/INVISIBLE: Nhân sự", true,
								XCTDKN.exist_nhan_su());
					} else if (sNhanSu.contains("@MARK INVISIBLE@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "VISIBLE/INVISIBLE: Nhân sự", false,
								XCTDKN.exist_nhan_su());
					}
				} else {
			...
			if (!sNgayTao.isEmpty()) {
				if (sNgayTao.startsWith("@MARK")) {
					if (sNgayTao.contains("@MARK HASFOCUS@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "HASFOCUS/NOTFOCUS: Ngày tạo", true,
								XCTDKN.focus_ngay_tao());
					} else if (sNgayTao.contains("@MARK NOTFOCUS@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "HASFOCUS/NOTFOCUS: Ngày tạo", false,
								XCTDKN.focus_ngay_tao());
					} else if (sNgayTao.contains("@MARK VISIBLE@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "VISIBLE/INVISIBLE: Ngày tạo", true,
								XCTDKN.exist_ngay_tao());
					} else if (sNgayTao.contains("@MARK INVISIBLE@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "VISIBLE/INVISIBLE: Ngày tạo", false,
								XCTDKN.exist_ngay_tao());
					}
				} else {
	...
	@Then("^Ket qua kiem tra thong tin ĐĂNG KÝ NGHỈ nhu \"([^\"]*)\"$")
	public void ket_qua_kiem_tra_thong_tin_DANG_KY_NGHI_nhu(String dataKey, List<List<String>> dataTable) {
	...
	public void Thuc_hien_XEM_CHI_TIET_DANG_KY_NGHI_nhu(String dataKey, List<List<String>> dataTable) throws Exception {
		String DATA_TABLE = "XEM_CHI_TIET_DANG_KY_NGHI_THUC_HIEN";
		SessionData.addDataTable(DATA_TABLE, dataTable, false);
		LinkedHashMap<Integer, List<String>> data = new LinkedHashMap<>();
		data = SessionData.getDataTbRowsByValEqualInCol(DATA_TABLE, "KEY", dataKey);
			...
			if (!sTuChoi.isEmpty()) {
				if (sTuChoi.startsWith("@MARK")) {
					if (sTuChoi.contains("@MARK DISABLED@@") || sTuChoi.contains("@MARK READONLY@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "ENABLED/DISABLED: Từ chối", false,
								XCTDKN.enabled_tu_choi());
					} else if (sTuChoi.contains("@MARK ENABLED@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "ENABLED/DISABLED: Từ chối", true,
								XCTDKN.enabled_tu_choi());
					} else if (sTuChoi.contains("@MARK HASFOCUS@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "HASFOCUS/NOTFOCUS: Từ chối", true,
								XCTDKN.focus_tu_choi());
					} else if (sTuChoi.contains("@MARK NOTFOCUS@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "HASFOCUS/NOTFOCUS: Từ chối", false,
								XCTDKN.focus_tu_choi());
					} else if (sTuChoi.contains("@MARK VISIBLE@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "VISIBLE/INVISIBLE: Từ chối", true,
								XCTDKN.exist_tu_choi());
					} else if (sTuChoi.contains("@MARK INVISIBLE@@")) {
						SessionData.addSoftAssertion(DATA_TABLE, sKey, "VISIBLE/INVISIBLE: Từ chối", false,
								XCTDKN.exist_tu_choi());
					}
	...
	public void Kiem_tra_cac_Control_hien_thi_day_du_tren_XEM_CHI_TIET_DANG_KY_NGHI() throws Exception {
		...
		SessionData.declareSoftAssertion(DATA_TABLE,
				"Kiem tra cac Control hien thi day du tren Xem chi tiết đăng ký nghỉ");
		SessionData.addSoftAssertion(DATA_TABLE, "H1_V", "VISIBLE/INVISIBLE: Phê duyệt", true,
				XCTDKN.exist_phe_duyet());
		...
		SessionData.addSoftAssertion(DATA_TABLE, "H5_V", "VISIBLE/INVISIBLE: Loại nghỉ", true,
				XCTDKN.exist_loai_nghi());
		...
		SessionData.addSoftAssertion(DATA_TABLE, "H8_V", "VISIBLE/INVISIBLE: Số ngày nghỉ", true,
				XCTDKN.exist_so_ngay_nghi());
